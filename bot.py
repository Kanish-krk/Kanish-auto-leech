...
def extract_links(message_text):
    ...
    magnet_pattern = r'(magnet:\?xt=urn:btih:[a-fA-F0-9]{1,60}).*'
    ...
    try:
        for mapping in CHANNEL_MAPPING:
            ...
            if message.chat.id == int(source_channel):
                ...
                modified_message_text = f"<b>{source_message.text}</b>".strip()

                for destination in destinations:
                    await client.send_message(chat_id=int(destination), text=modified_message_text)
                    await asyncio.sleep(5)

                logger.info("Forwarded a modified message from %s to %s", source_channel, destinations)
